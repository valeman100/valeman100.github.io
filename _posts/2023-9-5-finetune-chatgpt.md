...
Finally last week OpenAI released the APIs to finetune the popular model on which is based ChatGPT!
...
OpenAI gives you the chance to train your LLM even if you don't know anything about the subject, you just need a bit of Python and an OpenAI account.
...
A times this question is underrated but in reality, it's a proper concert to open before embarking on the finetuning way.
...
The main course of the receipt is the goodness of the dataset. It must be:
- informative, i.e. the examples need to have the prompts that give you the best answer without finetune, and the ground truth completion must be unique.
- correctly formatted.

While for the first point I can't help you with the second one I show you the correct packing for your data.
...
OpenAI APIs require your data to follow the format above. Here I suppose your dataset in a pandas dataframe format and refactor it in a list containing the examples of your training. Then we divide it into training (90%) and validation (10%)
...
Create a finetuning job passing the dataset IDs, one of the available tunable models (I advise you to choose 'gpt-3.5-turbo' i.e. the one used for ChatGPT, it's also the latest released), a suffix for your job. It's also possible to set the number of epochs which by default are set to 3.
...
Finally, we printed some useful info which include also the finetuning prices (referred to the specific model we selected).
...
At the end of the training we are ready to store the model ID we'll use to call the classic OpenAI completion API.
...
Finally, some tasks can't be achieved with some models. For example, don't expect GPT4 results finetuning a 7B model. 
